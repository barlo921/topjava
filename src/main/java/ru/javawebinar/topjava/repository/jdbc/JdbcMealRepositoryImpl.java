...
public class JdbcMealRepositoryImpl implements MealRepository {
    ...
    public Meal save(Meal meal, int userId) {
        ...
        if (meal.isNew()) {
            Number neKey = insertUser.executeAndReturnKey(map);
            meal.setId((Integer) neKey);
    ...
    public boolean delete(int id, int userId) {
        return jdbcTemplate.update("DELETE FROM meals meal WHERE meal.id=? AND meal.user_id=?", id, userId) != 0;
    }
    ...
    public List<Meal> getBetween(LocalDateTime startDate, LocalDateTime endDate, int userId) {
        ...
        meals.removeIf(meal -> !Util.isBetween(meal.getDateTime(), startDate, endDate));
        return meals;
    }
}
